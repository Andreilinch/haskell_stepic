-- Haskell - чисто функциональный язык программирования. Это значит, что в нем нет
-- понятия изменяемой переменной, следовательно, невовозможно определить понятие цикла
-- как в итеративыных языках, ведь нет никакой возможности отличить одну итерацию цикла от другой!
...
-- Определите функцию, вычисляющую двойной факториал, то есть произведение натуральных
-- чисел, не превосходящих заданного числа и имеющих ту же четность.
-- Например: 7!!=7\cdot 5\cdot 3\cdot 17!!=7⋅5⋅3⋅1, 
-- 8!!=8\cdot 6\cdot 4\cdot 28!!=8⋅6⋅4⋅2.
...
-- undefined обычно применяют для еще не написанных частей программы если мы не знаем
-- какой будет тип у каких-то элементов, так как элемент undefined является
-- елементом любого типа и проверка типов гарантированно пройдет
...
-- можно переписать это вообще без сопостовления с образцом,
-- только с использованием охранных выражений
...
-- я запустил те же программы на входе 2×10^5. Время работы кода составило 16,1 и 16,7
-- секунд (с аккумулятором больше), а время работы сборщика мусора — 1,9 и 0,4 секунд
-- (с аккумулятором меньше). То есть суммарно, опять же, вариант с аккумулятором
-- незначительно, но быстрее. Зато использование памяти с аккумулятором было 3 Мб,
-- а в наивном случае — 14 (!).
...
-- можно переписать это выражение используя локальное связывание с помощью конструкции 
-- let...in. Это будет лучше так как фунция helper нужна только для функции факториал
-- и не нужна в определении функций на глобальном уровне
...
-- то есть мы локально определили функцию helper
