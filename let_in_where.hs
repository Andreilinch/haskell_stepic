...
-- связываение можно осуществлять не только с помощью let_in, но и с помощью конструкции 
-- where
...
-- основное отличие двух походов в том, что let_in является полноценным выраженеим,
-- в то время как where может быть использовано только в глобальной части тела функции и
-- выражениме не является. Это позволяет делать разные конструкции с охранными выраженимяи
-- (см. recursion -> factorial7)
