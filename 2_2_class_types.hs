...
-- Представителем класса типов можно обьявлять не только конкретный тип, но и некоторые
-- полиморфные типы
-- Например, тип двухэлементного кортежа - пары. Как проверить две пары на равентсво?

instance (Eq a, Eq b) => Eq (a, b) where -- контекст в обявлении представителя
